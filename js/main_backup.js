...
function apiConfigured() {
  ...
  return !!cfg.provider; // token optional
}
...
function initApiKeyTab() {
  ...
  const warnEl = document.getElementById("api-warning");
  const statusEl = document.getElementById("api-status");

  if (!providerEl || !tokenEl || !saveBtn || !clearBtn) return;
  ...
  function renderWarn() {
    if (!warnEl) return;
    if (!apiConfigured()) warnEl.classList.remove("hidden");
  ...
  function setStatus(msg, ok) {
    ...
    statusEl.className = "mt-3 text-sm " + (ok ? "text-green-400" : "text-red-400");
  ...
  saveBtn.addEventListener("click", () => {
    ...
    if (!provider) {
      //setStatus("Please select a provider first.", false);
      alertError("Please select a provider first.")
    }

    setApiConfig({ provider, token });

    renderWarn();
    //renderApiBanner();
    alertSuccess(token ? "Saved provider + token." : "Saved provider (no token; rate-limited).")
  });
...
function requireApiConfigured() {
  if (!apiConfigured()) {
    renderApiBanner();
    throw new Error("API not configured. Go to API Key tab.");
  }
}
 ...
 * Blockcypher
 */
function blockcypherBase(currency) {
  // currency: "bitcoin" | "litecoin"
  if (currency === "bitcoin") return "https://api.blockcypher.com/v1/btc/main";
  if (currency === "litecoin") return "https://api.blockcypher.com/v1/ltc/main";
  throw new Error("Unsupported currency");
}

function mempoolBase(currency) {
  // NOTE: mempool.space is BTC
  if (currency === "bitcoin") return "https://mempool.space/api";

  // Litecoin mempool explorer commonly used:
  if (currency === "litecoin") return "https://litecoinspace.org/api";
...
function withToken(url) {
  const cfg = getApiConfig();
  if (cfg.provider !== "blockcypher") return url;
  if (!cfg.token) return url;
  return url + (url.includes("?") ? "&" : "?") + "token=" + encodeURIComponent(cfg.token);
}
...
async function balanceApi() {
    ...
    if (allaccount) {
      ...
      for(let i in allaccount) {
        ...
        } else if (allaccount[i].currency === "bitcoin" || allaccount[i].currency === "litecoin") {
          try {
            // currency passed to apiGetBalance is "bitcoin" | "litecoin"
            const b = await apiGetBalance(allaccount[i].address, allaccount[i].currency);

            // keep your existing behavior: use confirmed balance
            allaccount[i].balance = b.confirmed;
            ...
            if (accounts) {
              ...
              const readmore = localStorage.getItem("accounts")
                ? JSON.parse(localStorage.getItem("accounts"))
                : null;

              if (readmore) listfile(readmore);
            }
          } catch (e) {
            console.log("Balance error:", e?.message || e);

            // Optional: show banner if provider not set
            // (apiGetBalance already calls requireApiConfigured which shows banner)
          }
      } else {
...
async function unspentApi(address) {
    if (address.currency === 'woodcoin') {
      ...
      try {
        ...
        // optional: surface nicer UI error if you want
        return { utxo: [], currency: address.currency, error: String(e?.message || e) };
      }
    } else {
      console.log("Chain not found")
      return
...
async function withdrawalApi(message) {
  ...
  const txid = message.transaction_id
	...
	if (message.currency === "woodcoin") {
      ...
      // IMPORTANT: pass "bitcoin" / "litecoin" (not BTC/LTC)
      const { txid, raw } = await apiBroadcastTx(rawTxHex, message.currency);
      ...
      // show UI result
      withdrawalBroadcastResponse({ message: { result: txid }, raw });

    } catch (e) {
      withdrawalBroadcastResponse({ error: { error: { message: String(e.message || e) } } });
    }
	} else {
		console.log("invalid currency")
		return
